...
OUT PBOOLEAN AccessStatus) asm("NtAccessCheck");

EXTERN_C NTSTATUS NtWorkerFactoryWorkerReady(
IN HANDLE WorkerFactoryHandle) asm("NtWorkerFactoryWorkerReady");
...
OUT PPORT_SECTION_READ ClientSharedMemory OPTIONAL) asm("NtAcceptConnectPort");

EXTERN_C NTSTATUS NtMapUserPhysicalPagesScatter(
IN PVOID VirtualAddresses,
IN PULONG NumberOfPages,
IN PULONG UserPfnArray OPTIONAL) asm("NtMapUserPhysicalPagesScatter");
...
IN NTSTATUS Status) asm("NtCallbackReturn");
...
IN PULONG Key OPTIONAL) asm("NtReadFile");
...
IN ULONG OutputBufferLength) asm("NtDeviceIoControlFile");
...
OUT PIO_STATUS_BLOCK IoStatusBlock,
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtRemoveIoCompletion");

EXTERN_C NTSTATUS NtReleaseSemaphore(
IN HANDLE SemaphoreHandle,
IN LONG ReleaseCount,
OUT PLONG PreviousCount OPTIONAL) asm("NtReleaseSemaphore");

EXTERN_C NTSTATUS NtReplyWaitReceivePort(
IN HANDLE PortHandle,
OUT PVOID PortContext OPTIONAL,
IN PPORT_MESSAGE ReplyMessage OPTIONAL,
OUT PPORT_MESSAGE ReceiveMessage) asm("NtReplyWaitReceivePort");

EXTERN_C NTSTATUS NtReplyPort(
IN HANDLE PortHandle,
IN PPORT_MESSAGE ReplyMessage) asm("NtReplyPort");

EXTERN_C NTSTATUS NtSetInformationThread(
IN HANDLE ThreadHandle,
IN THREADINFOCLASS ThreadInformationClass,
IN PVOID ThreadInformation,
IN ULONG ThreadInformationLength) asm("NtSetInformationThread");

EXTERN_C NTSTATUS NtSetEvent(
IN HANDLE EventHandle,
OUT PULONG PreviousState OPTIONAL) asm("NtSetEvent");
...
IN ULONG Length,
IN FILE_INFORMATION_CLASS FileInformationClass) asm("NtQueryInformationFile");

EXTERN_C NTSTATUS NtOpenKey(
OUT PHANDLE KeyHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenKey");
...
IN ULONG Length,
OUT PULONG ResultLength) asm("NtEnumerateValueKey");

EXTERN_C NTSTATUS NtFindAtom(
IN PWSTR AtomName OPTIONAL,
IN ULONG Length,
OUT PUSHORT Atom OPTIONAL) asm("NtFindAtom");

EXTERN_C NTSTATUS NtQueryDefaultLocale(
IN BOOLEAN UserProfile,
OUT PLCID DefaultLocaleId) asm("NtQueryDefaultLocale");
...
IN ULONG Length,
OUT PULONG ResultLength) asm("NtQueryKey");
...
OUT PVOID KeyValueInformation OPTIONAL,
IN ULONG Length,
OUT PULONG ResultLength) asm("NtQueryValueKey");
...
IN BOOLEAN Alertable,
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtWaitForMultipleObjects32");
...
IN PULONG Key OPTIONAL) asm("NtWriteFileGather");
...
OUT PULONG Disposition OPTIONAL) asm("NtCreateKey");
...
IN HANDLE PortHandle,
IN PPORT_MESSAGE Message) asm("NtImpersonateClientOfPort");

EXTERN_C NTSTATUS NtReleaseMutant(
IN HANDLE MutantHandle,
OUT PULONG PreviousCount OPTIONAL) asm("NtReleaseMutant");
...
OUT PULONG ReturnLength) asm("NtQueryInformationToken");

EXTERN_C NTSTATUS NtRequestWaitReplyPort(
IN HANDLE PortHandle,
IN PPORT_MESSAGE RequestMessage,
OUT PPORT_MESSAGE ReplyMessage) asm("NtRequestWaitReplyPort");
...
OUT PHANDLE TokenHandle) asm("NtOpenThreadToken");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryInformationThread");
...
IN ULONG Length,
IN FILE_INFORMATION_CLASS FileInformationClass) asm("NtSetInformationFile");
...
OUT PBOOLEAN GenerateOnClose) asm("NtAccessCheckAndAuditAlarm");
...
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtReplyWaitReceivePortEx");
...
IN HANDLE EventHandle) asm("NtSetEventBoostPriority");
...
IN ULONG Length,
IN PLARGE_INTEGER ByteOffset OPTIONAL,
IN PULONG Key OPTIONAL) asm("NtReadFileScatter");
...
OUT PHANDLE TokenHandle) asm("NtOpenThreadTokenEx");

EXTERN_C NTSTATUS NtOpenProcessTokenEx(
IN HANDLE ProcessHandle,
IN ACCESS_MASK DesiredAccess,
IN ULONG HandleAttributes,
OUT PHANDLE TokenHandle) asm("NtOpenProcessTokenEx");

EXTERN_C NTSTATUS NtQueryPerformanceCounter(
OUT PLARGE_INTEGER PerformanceCounter,
OUT PLARGE_INTEGER PerformanceFrequency OPTIONAL) asm("NtQueryPerformanceCounter");
...
OUT PVOID KeyInformation OPTIONAL,
IN ULONG Length,
OUT PULONG ResultLength) asm("NtEnumerateKey");
...
IN ULONG OpenOptions) asm("NtOpenFile");

EXTERN_C NTSTATUS NtDelayExecution(
IN BOOLEAN Alertable,
IN PLARGE_INTEGER DelayInterval) asm("NtDelayExecution");
...
IN BOOLEAN RestartScan) asm("NtQueryDirectoryFile");
...
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenSection");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryTimer");
...
IN ULONG InputBufferLength,
OUT PVOID OutputBuffer OPTIONAL,
IN ULONG OutputBufferLength) asm("NtFsControlFile");
...
IN PUNICODE_STRING SubsystemName,
IN PVOID HandleId OPTIONAL,
IN BOOLEAN GenerateOnClose) asm("NtCloseObjectAuditAlarm");
...
IN POBJECT_ATTRIBUTES ObjectAttributes,
OUT PFILE_BASIC_INFORMATION FileInformation) asm("NtQueryAttributesFile");

EXTERN_C NTSTATUS NtClearEvent(
IN HANDLE EventHandle) asm("NtClearEvent");
...
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenEvent");
...
OUT PHANDLE NewTokenHandle) asm("NtDuplicateToken");

EXTERN_C NTSTATUS NtContinue(
IN PCONTEXT ContextRecord,
IN BOOLEAN TestAlert) asm("NtContinue");

EXTERN_C NTSTATUS NtQueryDefaultUILanguage(
OUT PLANGID DefaultUILanguageId) asm("NtQueryDefaultUILanguage");
...
EXTERN_C NTSTATUS NtYieldExecution() asm("NtYieldExecution");

EXTERN_C NTSTATUS NtAddAtom(
IN PWSTR AtomName OPTIONAL,
IN ULONG Length,
OUT PUSHORT Atom OPTIONAL) asm("NtAddAtom");
...
IN BOOLEAN InitialState) asm("NtCreateEvent");
...
IN ULONG Length,
IN FSINFOCLASS FsInformationClass) asm("NtQueryVolumeInformationFile");
...
IN HANDLE FileHandle,
OUT PIO_STATUS_BLOCK IoStatusBlock) asm("NtFlushBuffersFile");

EXTERN_C NTSTATUS NtApphelpCacheControl(
IN APPHELPCACHESERVICECLASS Service,
IN PVOID ServiceData) asm("NtApphelpCacheControl");
...
IN ULONG JobMemberLevel) asm("NtCreateProcessEx");
...
IN BOOLEAN CreateSuspended) asm("NtCreateThread");

EXTERN_C NTSTATUS NtIsProcessInJob(
IN HANDLE ProcessHandle,
IN HANDLE JobHandle OPTIONAL) asm("NtIsProcessInJob");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQuerySection");
...
IN HANDLE ThreadHandle,
IN NTSTATUS ExitStatus) asm("NtTerminateThread");
...
OUT PVOID Buffer,
IN ULONG BufferSize,
OUT PULONG NumberOfBytesRead OPTIONAL) asm("NtReadRequestData");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryEvent");
...
IN PVOID Buffer,
IN ULONG Length,
OUT PULONG ResultLength OPTIONAL) asm("NtWriteRequestData");

EXTERN_C NTSTATUS NtOpenDirectoryObject(
OUT PHANDLE DirectoryHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenDirectoryObject");
...
OUT PBOOLEAN GenerateOnClose) asm("NtAccessCheckByTypeAndAuditAlarm");
...
IN WAIT_TYPE WaitType,
IN BOOLEAN Alertable,
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtWaitForMultipleObjects");

EXTERN_C NTSTATUS NtSetInformationObject(
IN HANDLE Handle,
IN OBJECT_INFORMATION_CLASS ObjectInformationClass,
IN PVOID ObjectInformation,
IN ULONG ObjectInformationLength) asm("NtSetInformationObject");

EXTERN_C NTSTATUS NtCancelIoFile(
IN HANDLE FileHandle,
OUT PIO_STATUS_BLOCK IoStatusBlock) asm("NtCancelIoFile");

EXTERN_C NTSTATUS NtTraceEvent(
IN HANDLE TraceHandle,
IN ULONG Flags,
IN ULONG FieldSize,
IN PVOID Fields) asm("NtTraceEvent");
...
IN ULONG InputBufferLength,
OUT PVOID OutputBuffer OPTIONAL,
IN ULONG OutputBufferLength) asm("NtPowerInformation");
...
IN ULONG Type,
IN PVOID SystemData,
IN ULONG DataSize) asm("NtSetValueKey");

EXTERN_C NTSTATUS NtCancelTimer(
IN HANDLE TimerHandle,
OUT PBOOLEAN CurrentState OPTIONAL) asm("NtCancelTimer");
...
OUT PBOOLEAN PreviousState OPTIONAL) asm("NtSetTimer");
...
IN OUT PULONG PrivilegeSetLength,
OUT PACCESS_MASK GrantedAccess,
OUT PULONG AccessStatus) asm("NtAccessCheckByType");
...
IN OUT PULONG PrivilegeSetLength,
OUT PACCESS_MASK GrantedAccess,
OUT PULONG AccessStatus) asm("NtAccessCheckByTypeResultList");
...
OUT PACCESS_MASK GrantedAccess,
OUT PULONG AccessStatus,
OUT PULONG GenerateOnClose) asm("NtAccessCheckByTypeResultListAndAuditAlarm");
...
OUT PACCESS_MASK GrantedAccess,
OUT PULONG AccessStatus,
OUT PULONG GenerateOnClose) asm("NtAccessCheckByTypeResultListAndAuditAlarmByHandle");

EXTERN_C NTSTATUS NtAcquireProcessActivityReference() asm("NtAcquireProcessActivityReference");

EXTERN_C NTSTATUS NtAddAtomEx(
IN PWSTR AtomName,
IN ULONG Length,
IN PRTL_ATOM Atom,
IN ULONG Flags) asm("NtAddAtomEx");

EXTERN_C NTSTATUS NtAddBootEntry(
IN PBOOT_ENTRY BootEntry,
OUT PULONG Id OPTIONAL) asm("NtAddBootEntry");

EXTERN_C NTSTATUS NtAddDriverEntry(
IN PEFI_DRIVER_ENTRY DriverEntry,
OUT PULONG Id OPTIONAL) asm("NtAddDriverEntry");
...
OUT PULONG ReturnLength) asm("NtAdjustGroupsToken");
...
OUT PULONG DeviceGroupsReturnBufferLength OPTIONAL) asm("NtAdjustTokenClaimsAndDeviceGroups");

EXTERN_C NTSTATUS NtAlertResumeThread(
IN HANDLE ThreadHandle,
OUT PULONG PreviousSuspendCount OPTIONAL) asm("NtAlertResumeThread");

EXTERN_C NTSTATUS NtAlertThread(
IN HANDLE ThreadHandle) asm("NtAlertThread");

EXTERN_C NTSTATUS NtAlertThreadByThreadId(
IN ULONG ThreadId) asm("NtAlertThreadByThreadId");

EXTERN_C NTSTATUS NtAllocateLocallyUniqueId(
OUT PLUID Luid) asm("NtAllocateLocallyUniqueId");

EXTERN_C NTSTATUS NtAllocateReserveObject(
OUT PHANDLE MemoryReserveHandle,
IN POBJECT_ATTRIBUTES ObjectAttributes,
IN MEMORY_RESERVE_TYPE Type) asm("NtAllocateReserveObject");

EXTERN_C NTSTATUS NtAllocateUserPhysicalPages(
IN HANDLE ProcessHandle,
IN OUT PULONG NumberOfPages,
OUT PULONG UserPfnArray) asm("NtAllocateUserPhysicalPages");

EXTERN_C NTSTATUS NtAllocateUuids(
OUT PLARGE_INTEGER Time,
OUT PULONG Range,
OUT PULONG Sequence,
OUT PUCHAR Seed) asm("NtAllocateUuids");
...
IN ULONG DataCount) asm("NtAllocateVirtualMemoryEx");
...
IN BOOLEAN AcceptConnection) asm("NtAlpcAcceptConnectPort");

EXTERN_C NTSTATUS NtAlpcCancelMessage(
IN HANDLE PortHandle,
IN ULONG Flags,
IN PALPC_CONTEXT_ATTR MessageContext) asm("NtAlpcCancelMessage");
...
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtAlpcConnectPort");
...
IN OUT PALPC_MESSAGE_ATTRIBUTES OutMessageAttributes OPTIONAL,
IN OUT PALPC_MESSAGE_ATTRIBUTES InMessageAttributes OPTIONAL,
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtAlpcConnectPortEx");

EXTERN_C NTSTATUS NtAlpcCreatePort(
OUT PHANDLE PortHandle,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL,
IN PALPC_PORT_ATTRIBUTES PortAttributes OPTIONAL) asm("NtAlpcCreatePort");
...
OUT PSIZE_T ActualSectionSize) asm("NtAlpcCreatePortSection");

EXTERN_C NTSTATUS NtAlpcCreateResourceReserve(
IN HANDLE PortHandle,
IN ULONG Flags,
IN SIZE_T MessageSize,
OUT PHANDLE ResourceId) asm("NtAlpcCreateResourceReserve");

EXTERN_C NTSTATUS NtAlpcCreateSectionView(
IN HANDLE PortHandle,
IN ULONG Flags,
IN OUT PALPC_DATA_VIEW_ATTR ViewAttributes) asm("NtAlpcCreateSectionView");

EXTERN_C NTSTATUS NtAlpcCreateSecurityContext(
IN HANDLE PortHandle,
IN ULONG Flags,
IN OUT PALPC_SECURITY_ATTR SecurityAttribute) asm("NtAlpcCreateSecurityContext");

EXTERN_C NTSTATUS NtAlpcDeletePortSection(
IN HANDLE PortHandle,
IN ULONG Flags,
IN HANDLE SectionHandle) asm("NtAlpcDeletePortSection");

EXTERN_C NTSTATUS NtAlpcDeleteResourceReserve(
IN HANDLE PortHandle,
IN ULONG Flags,
IN HANDLE ResourceId) asm("NtAlpcDeleteResourceReserve");

EXTERN_C NTSTATUS NtAlpcDeleteSectionView(
IN HANDLE PortHandle,
IN ULONG Flags,
IN PVOID ViewBase) asm("NtAlpcDeleteSectionView");

EXTERN_C NTSTATUS NtAlpcDeleteSecurityContext(
IN HANDLE PortHandle,
IN ULONG Flags,
IN HANDLE ContextHandle) asm("NtAlpcDeleteSecurityContext");

EXTERN_C NTSTATUS NtAlpcDisconnectPort(
IN HANDLE PortHandle,
IN ULONG Flags) asm("NtAlpcDisconnectPort");

EXTERN_C NTSTATUS NtAlpcImpersonateClientContainerOfPort(
IN HANDLE PortHandle,
IN PPORT_MESSAGE Message,
IN ULONG Flags) asm("NtAlpcImpersonateClientContainerOfPort");

EXTERN_C NTSTATUS NtAlpcImpersonateClientOfPort(
IN HANDLE PortHandle,
IN PPORT_MESSAGE Message,
IN PVOID Flags) asm("NtAlpcImpersonateClientOfPort");
...
IN ULONG Flags,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtAlpcOpenSenderProcess");
...
IN ULONG Flags,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtAlpcOpenSenderThread");
...
IN ULONG Length,
OUT PULONG ReturnLength OPTIONAL) asm("NtAlpcQueryInformation");
...
IN ULONG Length,
OUT PULONG ReturnLength OPTIONAL) asm("NtAlpcQueryInformationMessage");

EXTERN_C NTSTATUS NtAlpcRevokeSecurityContext(
IN HANDLE PortHandle,
IN ULONG Flags,
IN HANDLE ContextHandle) asm("NtAlpcRevokeSecurityContext");
...
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtAlpcSendWaitReceivePort");

EXTERN_C NTSTATUS NtAlpcSetInformation(
IN HANDLE PortHandle,
IN ALPC_PORT_INFORMATION_CLASS PortInformationClass,
IN PVOID PortInformation OPTIONAL,
IN ULONG Length) asm("NtAlpcSetInformation");

EXTERN_C NTSTATUS NtAreMappedFilesTheSame(
IN PVOID File1MappedAsAnImage,
IN PVOID File2MappedAsFile) asm("NtAreMappedFilesTheSame");

EXTERN_C NTSTATUS NtAssignProcessToJobObject(
IN HANDLE JobHandle,
IN HANDLE ProcessHandle) asm("NtAssignProcessToJobObject");
...
OUT PBOOLEAN AlreadySignaled OPTIONAL) asm("NtAssociateWaitCompletionPacket");

EXTERN_C NTSTATUS NtCancelIoFileEx(
IN HANDLE FileHandle,
IN PIO_STATUS_BLOCK IoRequestToCancel OPTIONAL,
OUT PIO_STATUS_BLOCK IoStatusBlock) asm("NtCancelIoFileEx");

EXTERN_C NTSTATUS NtCancelSynchronousIoFile(
IN HANDLE ThreadHandle,
IN PIO_STATUS_BLOCK IoRequestToCancel OPTIONAL,
OUT PIO_STATUS_BLOCK IoStatusBlock) asm("NtCancelSynchronousIoFile");

EXTERN_C NTSTATUS NtCancelTimer2(
IN HANDLE TimerHandle,
IN PT2_CANCEL_PARAMETERS Parameters) asm("NtCancelTimer2");

EXTERN_C NTSTATUS NtCancelWaitCompletionPacket(
IN HANDLE WaitCompletionPacketHandle,
IN BOOLEAN RemoveSignaledPacket) asm("NtCancelWaitCompletionPacket");

EXTERN_C NTSTATUS NtCommitComplete(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtCommitComplete");

EXTERN_C NTSTATUS NtCommitEnlistment(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtCommitEnlistment");

EXTERN_C NTSTATUS NtCommitRegistryTransaction(
IN HANDLE RegistryHandle,
IN BOOL Wait) asm("NtCommitRegistryTransaction");

EXTERN_C NTSTATUS NtCommitTransaction(
IN HANDLE TransactionHandle,
IN BOOLEAN Wait) asm("NtCommitTransaction");

EXTERN_C NTSTATUS NtCompactKeys(
IN ULONG Count,
IN HANDLE KeyArray) asm("NtCompactKeys");

EXTERN_C NTSTATUS NtCompareObjects(
IN HANDLE FirstObjectHandle,
IN HANDLE SecondObjectHandle) asm("NtCompareObjects");

EXTERN_C NTSTATUS NtCompareSigningLevels(
IN ULONG UnknownParameter1,
IN ULONG UnknownParameter2) asm("NtCompareSigningLevels");

EXTERN_C NTSTATUS NtCompareTokens(
IN HANDLE FirstTokenHandle,
IN HANDLE SecondTokenHandle,
OUT PBOOLEAN Equal) asm("NtCompareTokens");

EXTERN_C NTSTATUS NtCompleteConnectPort(
IN HANDLE PortHandle) asm("NtCompleteConnectPort");

EXTERN_C NTSTATUS NtCompressKey(
IN HANDLE Key) asm("NtCompressKey");
...
IN OUT PULONG ConnectionInformationLength OPTIONAL) asm("NtConnectPort");
...
IN ULONG UnknownParameter4) asm("NtConvertBetweenAuxiliaryCounterAndPerformanceCounter");

EXTERN_C NTSTATUS NtCreateDebugObject(
OUT PHANDLE DebugObjectHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes,
IN ULONG Flags) asm("NtCreateDebugObject");

EXTERN_C NTSTATUS NtCreateDirectoryObject(
OUT PHANDLE DirectoryHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtCreateDirectoryObject");
...
IN ULONG Flags) asm("NtCreateDirectoryObjectEx");
...
OUT PULONG EnclaveError OPTIONAL) asm("NtCreateEnclave");
...
IN PVOID EnlistmentKey OPTIONAL) asm("NtCreateEnlistment");

EXTERN_C NTSTATUS NtCreateEventPair(
OUT PHANDLE EventPairHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL) asm("NtCreateEventPair");

EXTERN_C NTSTATUS NtCreateIRTimer(
OUT PHANDLE TimerHandle,
IN ACCESS_MASK DesiredAccess) asm("NtCreateIRTimer");

EXTERN_C NTSTATUS NtCreateIoCompletion(
OUT PHANDLE IoCompletionHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL,
IN ULONG Count OPTIONAL) asm("NtCreateIoCompletion");

EXTERN_C NTSTATUS NtCreateJobObject(
OUT PHANDLE JobHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL) asm("NtCreateJobObject");

EXTERN_C NTSTATUS NtCreateJobSet(
IN ULONG NumJob,
IN PJOB_SET_ARRAY UserJobSet,
IN ULONG Flags) asm("NtCreateJobSet");
...
IN ULONG CreateOptions,
IN HANDLE TransactionHandle,
OUT PULONG Disposition OPTIONAL) asm("NtCreateKeyTransacted");

EXTERN_C NTSTATUS NtCreateKeyedEvent(
OUT PHANDLE KeyedEventHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL,
IN ULONG Flags) asm("NtCreateKeyedEvent");
...
IN HANDLE Handles OPTIONAL) asm("NtCreateLowBoxToken");
...
IN PLARGE_INTEGER ReadTimeout) asm("NtCreateMailslotFile");

EXTERN_C NTSTATUS NtCreateMutant(
OUT PHANDLE MutantHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL,
IN BOOLEAN InitialOwner) asm("NtCreateMutant");
...
IN PLARGE_INTEGER DefaultTimeout OPTIONAL) asm("NtCreateNamedPipeFile");

EXTERN_C NTSTATUS NtCreatePagingFile(
IN PUNICODE_STRING PageFileName,
IN PULARGE_INTEGER MinimumSize,
IN PULARGE_INTEGER MaximumSize,
IN ULONG Priority) asm("NtCreatePagingFile");

EXTERN_C NTSTATUS NtCreatePartition(
OUT PHANDLE PartitionHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL,
IN ULONG PreferredNode) asm("NtCreatePartition");
...
IN ULONG MaxPoolUsage OPTIONAL) asm("NtCreatePort");

EXTERN_C NTSTATUS NtCreatePrivateNamespace(
OUT PHANDLE NamespaceHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes,
IN PVOID BoundaryDescriptor) asm("NtCreatePrivateNamespace");
...
IN ULONG Affinity) asm("NtCreateProfile");
...
IN PGROUP_AFFINITY GroupAffinity) asm("NtCreateProfileEx");

EXTERN_C NTSTATUS NtCreateRegistryTransaction(
OUT PHANDLE Handle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes,
IN DWORD Flags) asm("NtCreateRegistryTransaction");
...
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL,
IN ULONG CreateOptions OPTIONAL,
IN PUNICODE_STRING Description OPTIONAL) asm("NtCreateResourceManager");
...
IN LONG MaximumCount) asm("NtCreateSemaphore");

EXTERN_C NTSTATUS NtCreateSymbolicLinkObject(
OUT PHANDLE LinkHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes,
IN PUNICODE_STRING LinkTarget) asm("NtCreateSymbolicLinkObject");
...
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL,
IN TIMER_TYPE TimerType) asm("NtCreateTimer");
...
IN ULONG Attributes,
IN ACCESS_MASK DesiredAccess) asm("NtCreateTimer2");
...
IN PTOKEN_SOURCE TokenSource) asm("NtCreateToken");
...
IN PTOKEN_PRIMARY_GROUP PrimaryGroup,
IN PTOKEN_DEFAULT_DACL DefaultDacl OPTIONAL,
IN PTOKEN_SOURCE TokenSource) asm("NtCreateTokenEx");
...
IN ULONG CreateOptions OPTIONAL,
IN ULONG CommitStrength OPTIONAL) asm("NtCreateTransactionManager");
...
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL) asm("NtCreateWaitCompletionPacket");
...
IN ULONG MaxConnectionInfoLength,
IN ULONG MaxMessageLength,
IN ULONG MaxPoolUsage OPTIONAL) asm("NtCreateWaitablePort");
...
IN PSECURITY_DESCRIPTOR SecurityDescriptor) asm("NtCreateWnfStateName");
...
IN SIZE_T StackCommit OPTIONAL) asm("NtCreateWorkerFactory");

EXTERN_C NTSTATUS NtDebugActiveProcess(
IN HANDLE ProcessHandle,
IN HANDLE DebugObjectHandle) asm("NtDebugActiveProcess");

EXTERN_C NTSTATUS NtDebugContinue(
IN HANDLE DebugObjectHandle,
IN PCLIENT_ID ClientId,
IN NTSTATUS ContinueStatus) asm("NtDebugContinue");

EXTERN_C NTSTATUS NtDeleteAtom(
IN USHORT Atom) asm("NtDeleteAtom");

EXTERN_C NTSTATUS NtDeleteBootEntry(
IN ULONG Id) asm("NtDeleteBootEntry");

EXTERN_C NTSTATUS NtDeleteDriverEntry(
IN ULONG Id) asm("NtDeleteDriverEntry");

EXTERN_C NTSTATUS NtDeleteFile(
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtDeleteFile");

EXTERN_C NTSTATUS NtDeleteKey(
IN HANDLE KeyHandle) asm("NtDeleteKey");

EXTERN_C NTSTATUS NtDeleteObjectAuditAlarm(
IN PUNICODE_STRING SubsystemName,
IN PVOID HandleId OPTIONAL,
IN BOOLEAN GenerateOnClose) asm("NtDeleteObjectAuditAlarm");

EXTERN_C NTSTATUS NtDeletePrivateNamespace(
IN HANDLE NamespaceHandle) asm("NtDeletePrivateNamespace");

EXTERN_C NTSTATUS NtDeleteValueKey(
IN HANDLE KeyHandle,
IN PUNICODE_STRING ValueName) asm("NtDeleteValueKey");

EXTERN_C NTSTATUS NtDeleteWnfStateData(
IN PCWNF_STATE_NAME StateName,
IN PVOID ExplicitScope OPTIONAL) asm("NtDeleteWnfStateData");

EXTERN_C NTSTATUS NtDeleteWnfStateName(
IN PCWNF_STATE_NAME StateName) asm("NtDeleteWnfStateName");

EXTERN_C NTSTATUS NtDisableLastKnownGood() asm("NtDisableLastKnownGood");

EXTERN_C NTSTATUS NtDisplayString(
IN PUNICODE_STRING String) asm("NtDisplayString");

EXTERN_C NTSTATUS NtDrawText(
IN PUNICODE_STRING String) asm("NtDrawText");

EXTERN_C NTSTATUS NtEnableLastKnownGood() asm("NtEnableLastKnownGood");

EXTERN_C NTSTATUS NtEnumerateBootEntries(
OUT PVOID Buffer OPTIONAL,
IN OUT PULONG BufferLength) asm("NtEnumerateBootEntries");

EXTERN_C NTSTATUS NtEnumerateDriverEntries(
OUT PVOID Buffer OPTIONAL,
IN OUT PULONG BufferLength) asm("NtEnumerateDriverEntries");

EXTERN_C NTSTATUS NtEnumerateSystemEnvironmentValuesEx(
IN ULONG InformationClass,
OUT PVOID Buffer,
IN OUT PULONG BufferLength) asm("NtEnumerateSystemEnvironmentValuesEx");
...
OUT PULONG ReturnLength) asm("NtEnumerateTransactionObject");

EXTERN_C NTSTATUS NtExtendSection(
IN HANDLE SectionHandle,
IN OUT PLARGE_INTEGER NewSectionSize) asm("NtExtendSection");
...
IN ULONG DataSize) asm("NtFilterBootOption");
...
OUT PHANDLE NewTokenHandle) asm("NtFilterToken");
...
OUT PHANDLE NewTokenHandle) asm("NtFilterTokenEx");
...
OUT PIO_STATUS_BLOCK IoStatusBlock) asm("NtFlushBuffersFileEx");

EXTERN_C NTSTATUS NtFlushInstallUILanguage(
IN LANGID InstallUILanguage,
IN ULONG SetComittedFlag) asm("NtFlushInstallUILanguage");

EXTERN_C NTSTATUS NtFlushInstructionCache(
IN HANDLE ProcessHandle,
IN PVOID BaseAddress OPTIONAL,
IN ULONG Length) asm("NtFlushInstructionCache");

EXTERN_C NTSTATUS NtFlushKey(
IN HANDLE KeyHandle) asm("NtFlushKey");

EXTERN_C NTSTATUS NtFlushProcessWriteBuffers() asm("NtFlushProcessWriteBuffers");

EXTERN_C NTSTATUS NtFlushVirtualMemory(
IN HANDLE ProcessHandle,
IN OUT PVOID BaseAddress,
IN OUT PULONG RegionSize,
OUT PIO_STATUS_BLOCK IoStatusBlock) asm("NtFlushVirtualMemory");

EXTERN_C NTSTATUS NtFlushWriteBuffer() asm("NtFlushWriteBuffer");

EXTERN_C NTSTATUS NtFreeUserPhysicalPages(
IN HANDLE ProcessHandle,
IN OUT PULONG NumberOfPages,
IN PULONG UserPfnArray) asm("NtFreeUserPhysicalPages");

EXTERN_C NTSTATUS NtFreezeRegistry(
IN ULONG TimeOutInSeconds) asm("NtFreezeRegistry");

EXTERN_C NTSTATUS NtFreezeTransactions(
IN PLARGE_INTEGER FreezeTimeout,
IN PLARGE_INTEGER ThawTimeout) asm("NtFreezeTransactions");
...
IN ULONG DescriptorSize) asm("NtGetCompleteWnfStateSubscription");

EXTERN_C NTSTATUS NtGetContextThread(
IN HANDLE ThreadHandle,
IN OUT PCONTEXT ThreadContext) asm("NtGetContextThread");

EXTERN_C NTSTATUS NtGetCurrentProcessorNumber() asm("NtGetCurrentProcessorNumber");

EXTERN_C NTSTATUS NtGetCurrentProcessorNumberEx(
OUT PULONG ProcNumber OPTIONAL) asm("NtGetCurrentProcessorNumberEx");

EXTERN_C NTSTATUS NtGetDevicePowerState(
IN HANDLE Device,
OUT PDEVICE_POWER_STATE State) asm("NtGetDevicePowerState");

EXTERN_C NTSTATUS NtGetMUIRegistryInfo(
IN ULONG Flags,
IN OUT PULONG DataSize,
OUT PVOID SystemData) asm("NtGetMUIRegistryInfo");
...
IN ULONG HandleAttributes,
IN ULONG Flags,
OUT PHANDLE NewThreadHandle) asm("NtGetNextThread");
...
OUT PULONG SectionSize) asm("NtGetNlsSectionPtr");
...
IN ULONG AsynchronousContext OPTIONAL) asm("NtGetNotificationResourceManager");
...
OUT PULONG Granularity) asm("NtGetWriteWatch");

EXTERN_C NTSTATUS NtImpersonateAnonymousToken(
IN HANDLE ThreadHandle) asm("NtImpersonateAnonymousToken");

EXTERN_C NTSTATUS NtImpersonateThread(
IN HANDLE ServerThreadHandle,
IN HANDLE ClientThreadHandle,
IN PSECURITY_QUALITY_OF_SERVICE SecurityQos) asm("NtImpersonateThread");
...
IN PVOID EnclaveInformation,
IN ULONG EnclaveInformationLength,
OUT PULONG EnclaveError OPTIONAL) asm("NtInitializeEnclave");

EXTERN_C NTSTATUS NtInitializeNlsFiles(
OUT PVOID BaseAddress,
OUT PLCID DefaultLocaleId,
OUT PLARGE_INTEGER DefaultCasingTableSize) asm("NtInitializeNlsFiles");

EXTERN_C NTSTATUS NtInitializeRegistry(
IN USHORT BootCondition) asm("NtInitializeRegistry");

EXTERN_C NTSTATUS NtInitiatePowerAction(
IN POWER_ACTION SystemAction,
IN SYSTEM_POWER_STATE LightestSystemState,
IN ULONG Flags,
IN BOOLEAN Asynchronous) asm("NtInitiatePowerAction");

EXTERN_C NTSTATUS NtIsSystemResumeAutomatic() asm("NtIsSystemResumeAutomatic");

EXTERN_C NTSTATUS NtIsUILanguageComitted() asm("NtIsUILanguageComitted");

EXTERN_C NTSTATUS NtListenPort(
IN HANDLE PortHandle,
OUT PPORT_MESSAGE ConnectionRequest) asm("NtListenPort");

EXTERN_C NTSTATUS NtLoadDriver(
IN PUNICODE_STRING DriverServiceName) asm("NtLoadDriver");
...
OUT PULONG EnclaveError OPTIONAL) asm("NtLoadEnclaveData");

EXTERN_C NTSTATUS NtLoadHotPatch(
IN PUNICODE_STRING HotPatchName,
IN ULONG LoadFlag) asm("NtLoadHotPatch");

EXTERN_C NTSTATUS NtLoadKey(
IN POBJECT_ATTRIBUTES TargetKey,
IN POBJECT_ATTRIBUTES SourceFile) asm("NtLoadKey");

EXTERN_C NTSTATUS NtLoadKey2(
IN POBJECT_ATTRIBUTES TargetKey,
IN POBJECT_ATTRIBUTES SourceFile,
IN ULONG Flags) asm("NtLoadKey2");
...
OUT PIO_STATUS_BLOCK IoStatus OPTIONAL) asm("NtLoadKeyEx");
...
IN BOOLEAN ExclusiveLock) asm("NtLockFile");

EXTERN_C NTSTATUS NtLockProductActivationKeys(
IN OUT PULONG pPrivateVer OPTIONAL,
OUT PULONG pSafeMode OPTIONAL) asm("NtLockProductActivationKeys");

EXTERN_C NTSTATUS NtLockRegistryKey(
IN HANDLE KeyHandle) asm("NtLockRegistryKey");

EXTERN_C NTSTATUS NtLockVirtualMemory(
IN HANDLE ProcessHandle,
IN PVOID BaseAddress,
IN PULONG RegionSize,
IN ULONG MapType) asm("NtLockVirtualMemory");

EXTERN_C NTSTATUS NtMakePermanentObject(
IN HANDLE Handle) asm("NtMakePermanentObject");

EXTERN_C NTSTATUS NtMakeTemporaryObject(
IN HANDLE Handle) asm("NtMakeTemporaryObject");
...
IN ULONG PartitionInformationLength) asm("NtManagePartition");
...
OUT PVOID BaseAddress OPTIONAL) asm("NtMapCMFModule");

EXTERN_C NTSTATUS NtMapUserPhysicalPages(
IN PVOID VirtualAddress,
IN PULONG NumberOfPages,
IN PULONG UserPfnArray OPTIONAL) asm("NtMapUserPhysicalPages");
...
IN ULONG Protect,
IN OUT PVOID DataBuffer OPTIONAL,
IN ULONG DataCount) asm("NtMapViewOfSectionEx");

EXTERN_C NTSTATUS NtModifyBootEntry(
IN PBOOT_ENTRY BootEntry) asm("NtModifyBootEntry");

EXTERN_C NTSTATUS NtModifyDriverEntry(
IN PEFI_DRIVER_ENTRY DriverEntry) asm("NtModifyDriverEntry");
...
IN BOOLEAN WatchTree) asm("NtNotifyChangeDirectoryFile");
...
IN DIRECTORY_NOTIFY_INFORMATION_CLASS DirectoryNotifyInformationClass OPTIONAL) asm("NtNotifyChangeDirectoryFileEx");
...
OUT PVOID Buffer OPTIONAL,
IN ULONG BufferSize,
IN BOOLEAN Asynchronous) asm("NtNotifyChangeKey");
...
OUT PVOID Buffer OPTIONAL,
IN ULONG BufferSize,
IN BOOLEAN Asynchronous) asm("NtNotifyChangeMultipleKeys");
...
IN ULONG PayloadSize) asm("NtNotifyChangeSession");
...
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL) asm("NtOpenEnlistment");

EXTERN_C NTSTATUS NtOpenEventPair(
OUT PHANDLE EventPairHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenEventPair");

EXTERN_C NTSTATUS NtOpenIoCompletion(
OUT PHANDLE IoCompletionHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenIoCompletion");

EXTERN_C NTSTATUS NtOpenJobObject(
OUT PHANDLE JobHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenJobObject");

EXTERN_C NTSTATUS NtOpenKeyEx(
OUT PHANDLE KeyHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes,
IN ULONG OpenOptions) asm("NtOpenKeyEx");

EXTERN_C NTSTATUS NtOpenKeyTransacted(
OUT PHANDLE KeyHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes,
IN HANDLE TransactionHandle) asm("NtOpenKeyTransacted");
...
IN HANDLE TransactionHandle) asm("NtOpenKeyTransactedEx");

EXTERN_C NTSTATUS NtOpenKeyedEvent(
OUT PHANDLE KeyedEventHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenKeyedEvent");

EXTERN_C NTSTATUS NtOpenMutant(
OUT PHANDLE MutantHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenMutant");
...
OUT PBOOLEAN GenerateOnClose) asm("NtOpenObjectAuditAlarm");

EXTERN_C NTSTATUS NtOpenPartition(
OUT PHANDLE PartitionHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenPartition");

EXTERN_C NTSTATUS NtOpenPrivateNamespace(
OUT PHANDLE NamespaceHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL,
IN PVOID BoundaryDescriptor) asm("NtOpenPrivateNamespace");
...
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenRegistryTransaction");
...
IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL) asm("NtOpenResourceManager");

EXTERN_C NTSTATUS NtOpenSemaphore(
OUT PHANDLE SemaphoreHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenSemaphore");

EXTERN_C NTSTATUS NtOpenSession(
OUT PHANDLE SessionHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenSession");

EXTERN_C NTSTATUS NtOpenSymbolicLinkObject(
OUT PHANDLE LinkHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenSymbolicLinkObject");
...
OUT PHANDLE TimerHandle,
IN ACCESS_MASK DesiredAccess,
IN POBJECT_ATTRIBUTES ObjectAttributes) asm("NtOpenTimer");
...
IN POBJECT_ATTRIBUTES ObjectAttributes,
IN LPGUID Uow,
IN HANDLE TmHandle OPTIONAL) asm("NtOpenTransaction");
...
IN ULONG OpenOptions OPTIONAL) asm("NtOpenTransactionManager");

EXTERN_C NTSTATUS NtPlugPlayControl(
IN PLUGPLAY_CONTROL_CLASS PnPControlClass,
IN OUT PVOID PnPControlData,
IN ULONG PnPControlDataLength) asm("NtPlugPlayControl");

EXTERN_C NTSTATUS NtPrePrepareComplete(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtPrePrepareComplete");

EXTERN_C NTSTATUS NtPrePrepareEnlistment(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtPrePrepareEnlistment");

EXTERN_C NTSTATUS NtPrepareComplete(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtPrepareComplete");

EXTERN_C NTSTATUS NtPrepareEnlistment(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtPrepareEnlistment");

EXTERN_C NTSTATUS NtPrivilegeCheck(
IN HANDLE ClientToken,
IN OUT PPRIVILEGE_SET RequiredPrivileges,
OUT PBOOLEAN Result) asm("NtPrivilegeCheck");
...
IN BOOLEAN AccessGranted) asm("NtPrivilegeObjectAuditAlarm");
...
IN HANDLE ClientToken,
IN PPRIVILEGE_SET Privileges,
IN BOOLEAN AccessGranted) asm("NtPrivilegedServiceAuditAlarm");

EXTERN_C NTSTATUS NtPropagationComplete(
IN HANDLE ResourceManagerHandle,
IN ULONG RequestCookie,
IN ULONG BufferLength,
IN PVOID Buffer) asm("NtPropagationComplete");

EXTERN_C NTSTATUS NtPropagationFailed(
IN HANDLE ResourceManagerHandle,
IN ULONG RequestCookie,
IN NTSTATUS PropStatus) asm("NtPropagationFailed");

EXTERN_C NTSTATUS NtPulseEvent(
IN HANDLE EventHandle,
OUT PULONG PreviousState OPTIONAL) asm("NtPulseEvent");

EXTERN_C NTSTATUS NtQueryAuxiliaryCounterFrequency(
OUT PULONGLONG lpAuxiliaryCounterFrequency) asm("NtQueryAuxiliaryCounterFrequency");

EXTERN_C NTSTATUS NtQueryBootEntryOrder(
OUT PULONG Ids OPTIONAL,
IN OUT PULONG Count) asm("NtQueryBootEntryOrder");

EXTERN_C NTSTATUS NtQueryBootOptions(
OUT PBOOT_OPTIONS BootOptions OPTIONAL,
IN OUT PULONG BootOptionsLength) asm("NtQueryBootOptions");

EXTERN_C NTSTATUS NtQueryDebugFilterState(
IN ULONG ComponentId,
IN ULONG Level) asm("NtQueryDebugFilterState");
...
IN FILE_INFORMATION_CLASS FileInformationClass,
IN ULONG QueryFlags,
IN PUNICODE_STRING FileName OPTIONAL) asm("NtQueryDirectoryFileEx");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryDirectoryObject");

EXTERN_C NTSTATUS NtQueryDriverEntryOrder(
IN PULONG Ids OPTIONAL,
IN OUT PULONG Count) asm("NtQueryDriverEntryOrder");
...
IN BOOLEAN RestartScan) asm("NtQueryEaFile");

EXTERN_C NTSTATUS NtQueryFullAttributesFile(
IN POBJECT_ATTRIBUTES ObjectAttributes,
OUT PFILE_NETWORK_OPEN_INFORMATION FileInformation) asm("NtQueryFullAttributesFile");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryInformationAtom");
...
OUT PVOID FileInformation,
IN ULONG Length,
IN FILE_INFORMATION_CLASS FileInformationClass) asm("NtQueryInformationByName");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryInformationEnlistment");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryInformationJobObject");
...
IN ULONG Length,
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryInformationPort");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryInformationResourceManager");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryInformationTransaction");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryInformationTransactionManager");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryInformationWorkerFactory");

EXTERN_C NTSTATUS NtQueryInstallUILanguage(
OUT PLANGID InstallUILanguageId) asm("NtQueryInstallUILanguage");

EXTERN_C NTSTATUS NtQueryIntervalProfile(
IN KPROFILE_SOURCE ProfileSource,
OUT PULONG Interval) asm("NtQueryIntervalProfile");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryIoCompletion");
...
IN ULONG DataSize,
OUT PULONG ResultDataSize) asm("NtQueryLicenseValue");
...
OUT PULONG RequiredBufferLength OPTIONAL) asm("NtQueryMultipleValueKey");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQueryMutant");

EXTERN_C NTSTATUS NtQueryOpenSubKeys(
IN POBJECT_ATTRIBUTES TargetKey,
OUT PULONG HandleCount) asm("NtQueryOpenSubKeys");

EXTERN_C NTSTATUS NtQueryOpenSubKeysEx(
IN POBJECT_ATTRIBUTES TargetKey,
IN ULONG BufferLength,
OUT PVOID Buffer,
OUT PULONG RequiredSize) asm("NtQueryOpenSubKeysEx");

EXTERN_C NTSTATUS NtQueryPortInformationProcess() asm("NtQueryPortInformationProcess");
...
IN BOOLEAN RestartScan) asm("NtQueryQuotaInformationFile");
...
OUT PVOID Buffer,
IN ULONG Length,
OUT PULONG ReturnLength) asm("NtQuerySecurityAttributesToken");
...
IN ULONG Length,
OUT PULONG LengthNeeded) asm("NtQuerySecurityObject");
...
IN ULONG_PTR UnknownParameter6) asm("NtQuerySecurityPolicy");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtQuerySemaphore");

EXTERN_C NTSTATUS NtQuerySymbolicLinkObject(
IN HANDLE LinkHandle,
IN OUT PUNICODE_STRING LinkTarget,
OUT PULONG ReturnedLength OPTIONAL) asm("NtQuerySymbolicLinkObject");

EXTERN_C NTSTATUS NtQuerySystemEnvironmentValue(
IN PUNICODE_STRING VariableName,
OUT PVOID VariableValue,
IN ULONG ValueLength,
OUT PULONG ReturnLength OPTIONAL) asm("NtQuerySystemEnvironmentValue");
...
OUT PULONG Attributes OPTIONAL) asm("NtQuerySystemEnvironmentValueEx");
...
IN ULONG SystemInformationLength,
OUT PULONG ReturnLength OPTIONAL) asm("NtQuerySystemInformationEx");

EXTERN_C NTSTATUS NtQueryTimerResolution(
OUT PULONG MaximumTime,
OUT PULONG MinimumTime,
OUT PULONG CurrentTime) asm("NtQueryTimerResolution");
...
OUT PVOID Buffer OPTIONAL,
IN OUT PULONG BufferSize) asm("NtQueryWnfStateData");
...
IN ULONG InfoBufferSize) asm("NtQueryWnfStateNameInformation");
...
IN PVOID ApcArgument1 OPTIONAL,
IN PVOID ApcArgument2 OPTIONAL,
IN PVOID ApcArgument3 OPTIONAL) asm("NtQueueApcThreadEx");

EXTERN_C NTSTATUS NtRaiseException(
IN PEXCEPTION_RECORD ExceptionRecord,
IN PCONTEXT ContextRecord,
IN BOOLEAN FirstChance) asm("NtRaiseException");
...
OUT PULONG Response) asm("NtRaiseHardError");

EXTERN_C NTSTATUS NtReadOnlyEnlistment(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtReadOnlyEnlistment");

EXTERN_C NTSTATUS NtRecoverEnlistment(
IN HANDLE EnlistmentHandle,
IN PVOID EnlistmentKey OPTIONAL) asm("NtRecoverEnlistment");

EXTERN_C NTSTATUS NtRecoverResourceManager(
IN HANDLE ResourceManagerHandle) asm("NtRecoverResourceManager");

EXTERN_C NTSTATUS NtRecoverTransactionManager(
IN HANDLE TransactionManagerHandle) asm("NtRecoverTransactionManager");
...
IN ULONG CreateOptions OPTIONAL) asm("NtRegisterProtocolAddressInformation");

EXTERN_C NTSTATUS NtRegisterThreadTerminatePort(
IN HANDLE PortHandle) asm("NtRegisterThreadTerminatePort");

EXTERN_C NTSTATUS NtReleaseKeyedEvent(
IN HANDLE KeyedEventHandle,
IN PVOID KeyValue,
IN BOOLEAN Alertable,
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtReleaseKeyedEvent");

EXTERN_C NTSTATUS NtReleaseWorkerFactoryWorker(
IN HANDLE WorkerFactoryHandle) asm("NtReleaseWorkerFactoryWorker");
...
IN PLARGE_INTEGER Timeout OPTIONAL,
IN BOOLEAN Alertable) asm("NtRemoveIoCompletionEx");

EXTERN_C NTSTATUS NtRemoveProcessDebug(
IN HANDLE ProcessHandle,
IN HANDLE DebugObjectHandle) asm("NtRemoveProcessDebug");

EXTERN_C NTSTATUS NtRenameKey(
IN HANDLE KeyHandle,
IN PUNICODE_STRING NewName) asm("NtRenameKey");

EXTERN_C NTSTATUS NtRenameTransactionManager(
IN PUNICODE_STRING LogFileName,
IN LPGUID ExistingTransactionManagerGuid) asm("NtRenameTransactionManager");

EXTERN_C NTSTATUS NtReplaceKey(
IN POBJECT_ATTRIBUTES NewFile,
IN HANDLE TargetHandle,
IN POBJECT_ATTRIBUTES OldFile) asm("NtReplaceKey");

EXTERN_C NTSTATUS NtReplacePartitionUnit(
IN PUNICODE_STRING TargetInstancePath,
IN PUNICODE_STRING SpareInstancePath,
IN ULONG Flags) asm("NtReplacePartitionUnit");

EXTERN_C NTSTATUS NtReplyWaitReplyPort(
IN HANDLE PortHandle,
IN OUT PPORT_MESSAGE ReplyMessage) asm("NtReplyWaitReplyPort");

EXTERN_C NTSTATUS NtRequestPort(
IN HANDLE PortHandle,
IN PPORT_MESSAGE RequestMessage) asm("NtRequestPort");

EXTERN_C NTSTATUS NtResetEvent(
IN HANDLE EventHandle,
OUT PULONG PreviousState OPTIONAL) asm("NtResetEvent");

EXTERN_C NTSTATUS NtResetWriteWatch(
IN HANDLE ProcessHandle,
IN PVOID BaseAddress,
IN ULONG RegionSize) asm("NtResetWriteWatch");

EXTERN_C NTSTATUS NtRestoreKey(
IN HANDLE KeyHandle,
IN HANDLE FileHandle,
IN ULONG Flags) asm("NtRestoreKey");

EXTERN_C NTSTATUS NtResumeProcess(
IN HANDLE ProcessHandle) asm("NtResumeProcess");

EXTERN_C NTSTATUS NtRevertContainerImpersonation() asm("NtRevertContainerImpersonation");

EXTERN_C NTSTATUS NtRollbackComplete(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtRollbackComplete");

EXTERN_C NTSTATUS NtRollbackEnlistment(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtRollbackEnlistment");

EXTERN_C NTSTATUS NtRollbackRegistryTransaction(
IN HANDLE RegistryHandle,
IN BOOL Wait) asm("NtRollbackRegistryTransaction");

EXTERN_C NTSTATUS NtRollbackTransaction(
IN HANDLE TransactionHandle,
IN BOOLEAN Wait) asm("NtRollbackTransaction");

EXTERN_C NTSTATUS NtRollforwardTransactionManager(
IN HANDLE TransactionManagerHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtRollforwardTransactionManager");

EXTERN_C NTSTATUS NtSaveKey(
IN HANDLE KeyHandle,
IN HANDLE FileHandle) asm("NtSaveKey");

EXTERN_C NTSTATUS NtSaveKeyEx(
IN HANDLE KeyHandle,
IN HANDLE FileHandle,
IN ULONG Format) asm("NtSaveKeyEx");

EXTERN_C NTSTATUS NtSaveMergedKeys(
IN HANDLE HighPrecedenceKeyHandle,
IN HANDLE LowPrecedenceKeyHandle,
IN HANDLE FileHandle) asm("NtSaveMergedKeys");
...
OUT PULONG MaxMessageLength OPTIONAL,
IN OUT PVOID ConnectionInformation OPTIONAL,
IN OUT PULONG ConnectionInformationLength OPTIONAL) asm("NtSecureConnectPort");

EXTERN_C NTSTATUS NtSerializeBoot() asm("NtSerializeBoot");

EXTERN_C NTSTATUS NtSetBootEntryOrder(
IN PULONG Ids,
IN ULONG Count) asm("NtSetBootEntryOrder");

EXTERN_C NTSTATUS NtSetBootOptions(
IN PBOOT_OPTIONS BootOptions,
IN ULONG FieldsToChange) asm("NtSetBootOptions");
...
IN PVOID LevelInformation OPTIONAL) asm("NtSetCachedSigningLevel2");
...
IN ULONG ComponentId,
IN ULONG Level,
IN BOOLEAN State) asm("NtSetDebugFilterState");

EXTERN_C NTSTATUS NtSetDefaultHardErrorPort(
IN HANDLE PortHandle) asm("NtSetDefaultHardErrorPort");

EXTERN_C NTSTATUS NtSetDefaultLocale(
IN BOOLEAN UserProfile,
IN LCID DefaultLocaleId) asm("NtSetDefaultLocale");

EXTERN_C NTSTATUS NtSetDefaultUILanguage(
IN LANGID DefaultUILanguageId) asm("NtSetDefaultUILanguage");

EXTERN_C NTSTATUS NtSetDriverEntryOrder(
IN PULONG Ids,
IN PULONG Count) asm("NtSetDriverEntryOrder");

EXTERN_C NTSTATUS NtSetEaFile(
IN HANDLE FileHandle,
OUT PIO_STATUS_BLOCK IoStatusBlock,
IN PFILE_FULL_EA_INFORMATION EaBuffer,
IN ULONG EaBufferSize) asm("NtSetEaFile");

EXTERN_C NTSTATUS NtSetHighEventPair(
IN HANDLE EventPairHandle) asm("NtSetHighEventPair");

EXTERN_C NTSTATUS NtSetHighWaitLowEventPair(
IN HANDLE EventPairHandle) asm("NtSetHighWaitLowEventPair");

EXTERN_C NTSTATUS NtSetIRTimer(
IN HANDLE TimerHandle,
IN PLARGE_INTEGER DueTime OPTIONAL) asm("NtSetIRTimer");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtSetInformationDebugObject");

EXTERN_C NTSTATUS NtSetInformationEnlistment(
IN HANDLE EnlistmentHandle,
IN ENLISTMENT_INFORMATION_CLASS EnlistmentInformationClass,
IN PVOID EnlistmentInformation,
IN ULONG EnlistmentInformationLength) asm("NtSetInformationEnlistment");

EXTERN_C NTSTATUS NtSetInformationJobObject(
IN HANDLE JobHandle,
IN JOBOBJECTINFOCLASS JobObjectInformationClass,
IN PVOID JobObjectInformation,
IN ULONG JobObjectInformationLength) asm("NtSetInformationJobObject");

EXTERN_C NTSTATUS NtSetInformationKey(
IN HANDLE KeyHandle,
IN KEY_SET_INFORMATION_CLASS KeySetInformationClass,
IN PVOID KeySetInformation,
IN ULONG KeySetInformationLength) asm("NtSetInformationKey");

EXTERN_C NTSTATUS NtSetInformationResourceManager(
IN HANDLE ResourceManagerHandle,
IN RESOURCEMANAGER_INFORMATION_CLASS ResourceManagerInformationClass,
IN PVOID ResourceManagerInformation,
IN ULONG ResourceManagerInformationLength) asm("NtSetInformationResourceManager");

EXTERN_C NTSTATUS NtSetInformationSymbolicLink(
IN HANDLE Handle,
IN ULONG Class,
IN PVOID Buffer,
IN ULONG BufferLength) asm("NtSetInformationSymbolicLink");

EXTERN_C NTSTATUS NtSetInformationToken(
IN HANDLE TokenHandle,
IN TOKEN_INFORMATION_CLASS TokenInformationClass,
IN PVOID TokenInformation,
IN ULONG TokenInformationLength) asm("NtSetInformationToken");

EXTERN_C NTSTATUS NtSetInformationTransaction(
IN HANDLE TransactionHandle,
IN TRANSACTIONMANAGER_INFORMATION_CLASS TransactionInformationClass,
IN PVOID TransactionInformation,
IN ULONG TransactionInformationLength) asm("NtSetInformationTransaction");

EXTERN_C NTSTATUS NtSetInformationTransactionManager(
IN HANDLE TransactionHandle,
IN TRANSACTION_INFORMATION_CLASS TransactionInformationClass,
IN PVOID TransactionInformation,
IN ULONG TransactionInformationLength) asm("NtSetInformationTransactionManager");
...
IN ULONG VmInformationLength) asm("NtSetInformationVirtualMemory");

EXTERN_C NTSTATUS NtSetInformationWorkerFactory(
IN HANDLE WorkerFactoryHandle,
IN WORKERFACTORYINFOCLASS WorkerFactoryInformationClass,
IN PVOID WorkerFactoryInformation,
IN ULONG WorkerFactoryInformationLength) asm("NtSetInformationWorkerFactory");

EXTERN_C NTSTATUS NtSetIntervalProfile(
IN ULONG Interval,
IN KPROFILE_SOURCE Source) asm("NtSetIntervalProfile");
...
IN ULONG NumberOfBytesTransfered) asm("NtSetIoCompletion");
...
IN PVOID ApcContext OPTIONAL,
IN NTSTATUS IoStatus,
IN ULONG_PTR IoStatusInformation) asm("NtSetIoCompletionEx");
...
IN ULONG Selector1,
IN ULONG Entry1Low,
IN ULONG Entry1Hi) asm("NtSetLdtEntries");

EXTERN_C NTSTATUS NtSetLowEventPair(
IN HANDLE EventPairHandle) asm("NtSetLowEventPair");

EXTERN_C NTSTATUS NtSetLowWaitHighEventPair(
IN HANDLE EventPairHandle) asm("NtSetLowWaitHighEventPair");

EXTERN_C NTSTATUS NtSetQuotaInformationFile(
IN HANDLE FileHandle,
OUT PIO_STATUS_BLOCK IoStatusBlock,
IN PFILE_USER_QUOTA_INFORMATION Buffer,
IN ULONG Length) asm("NtSetQuotaInformationFile");

EXTERN_C NTSTATUS NtSetSecurityObject(
IN HANDLE ObjectHandle,
IN SECURITY_INFORMATION SecurityInformationClass,
IN PSECURITY_DESCRIPTOR DescriptorBuffer) asm("NtSetSecurityObject");

EXTERN_C NTSTATUS NtSetSystemEnvironmentValue(
IN PUNICODE_STRING VariableName,
IN PUNICODE_STRING Value) asm("NtSetSystemEnvironmentValue");
...
IN ULONG ValueLength,
IN ULONG Attributes) asm("NtSetSystemEnvironmentValueEx");

EXTERN_C NTSTATUS NtSetSystemInformation(
IN SYSTEM_INFORMATION_CLASS SystemInformationClass,
IN PVOID SystemInformation,
IN ULONG SystemInformationLength) asm("NtSetSystemInformation");

EXTERN_C NTSTATUS NtSetSystemPowerState(
IN POWER_ACTION SystemAction,
IN SYSTEM_POWER_STATE MinSystemState,
IN ULONG Flags) asm("NtSetSystemPowerState");

EXTERN_C NTSTATUS NtSetSystemTime(
IN PLARGE_INTEGER SystemTime,
OUT PLARGE_INTEGER PreviousTime OPTIONAL) asm("NtSetSystemTime");

EXTERN_C NTSTATUS NtSetThreadExecutionState(
IN EXECUTION_STATE ExecutionState,
OUT PEXECUTION_STATE PreviousExecutionState) asm("NtSetThreadExecutionState");

EXTERN_C NTSTATUS NtSetTimer2(
IN HANDLE TimerHandle,
IN PLARGE_INTEGER DueTime,
IN PLARGE_INTEGER Period OPTIONAL,
IN PT2_SET_PARAMETERS Parameters) asm("NtSetTimer2");
...
IN ULONG TimerSetInformationLength) asm("NtSetTimerEx");

EXTERN_C NTSTATUS NtSetTimerResolution(
IN ULONG DesiredResolution,
IN BOOLEAN SetResolution,
OUT PULONG CurrentResolution) asm("NtSetTimerResolution");

EXTERN_C NTSTATUS NtSetUuidSeed(
IN PUCHAR Seed) asm("NtSetUuidSeed");
...
IN ULONG Length,
IN FSINFOCLASS FileSystemInformationClass) asm("NtSetVolumeInformationFile");

EXTERN_C NTSTATUS NtSetWnfProcessNotificationEvent(
IN HANDLE NotificationEvent) asm("NtSetWnfProcessNotificationEvent");

EXTERN_C NTSTATUS NtShutdownSystem(
IN SHUTDOWN_ACTION Action) asm("NtShutdownSystem");

EXTERN_C NTSTATUS NtShutdownWorkerFactory(
IN HANDLE WorkerFactoryHandle,
IN OUT PLONG PendingWorkerCount) asm("NtShutdownWorkerFactory");

EXTERN_C NTSTATUS NtSignalAndWaitForSingleObject(
IN HANDLE hObjectToSignal,
IN HANDLE hObjectToWaitOn,
IN BOOLEAN bAlertable,
IN PLARGE_INTEGER dwMilliseconds OPTIONAL) asm("NtSignalAndWaitForSingleObject");

EXTERN_C NTSTATUS NtSinglePhaseReject(
IN HANDLE EnlistmentHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtSinglePhaseReject");

EXTERN_C NTSTATUS NtStartProfile(
IN HANDLE ProfileHandle) asm("NtStartProfile");

EXTERN_C NTSTATUS NtStopProfile(
IN HANDLE ProfileHandle) asm("NtStopProfile");

EXTERN_C NTSTATUS NtSubscribeWnfStateChange(
IN PCWNF_STATE_NAME StateName,
IN WNF_CHANGE_STAMP ChangeStamp OPTIONAL,
IN ULONG EventMask,
OUT PLARGE_INTEGER SubscriptionId OPTIONAL) asm("NtSubscribeWnfStateChange");

EXTERN_C NTSTATUS NtSuspendProcess(
IN HANDLE ProcessHandle) asm("NtSuspendProcess");
...
OUT PULONG ReturnLength OPTIONAL) asm("NtSystemDebugControl");

EXTERN_C NTSTATUS NtTerminateEnclave(
IN PVOID BaseAddress,
IN BOOLEAN WaitForThread) asm("NtTerminateEnclave");

EXTERN_C NTSTATUS NtTerminateJobObject(
IN HANDLE JobHandle,
IN NTSTATUS ExitStatus) asm("NtTerminateJobObject");

EXTERN_C NTSTATUS NtTestAlert() asm("NtTestAlert");

EXTERN_C NTSTATUS NtThawRegistry() asm("NtThawRegistry");

EXTERN_C NTSTATUS NtThawTransactions() asm("NtThawTransactions");
...
OUT PVOID OutputBuffer OPTIONAL,
IN ULONG OutputBufferLength,
OUT PULONG ReturnLength) asm("NtTraceControl");

EXTERN_C NTSTATUS NtTranslateFilePath(
IN PFILE_PATH InputFilePath,
IN ULONG OutputType,
OUT PFILE_PATH OutputFilePath OPTIONAL,
IN OUT PULONG OutputFilePathLength OPTIONAL) asm("NtTranslateFilePath");

EXTERN_C NTSTATUS NtUmsThreadYield(
IN PVOID SchedulerParam) asm("NtUmsThreadYield");

EXTERN_C NTSTATUS NtUnloadDriver(
IN PUNICODE_STRING DriverServiceName) asm("NtUnloadDriver");

EXTERN_C NTSTATUS NtUnloadKey(
IN POBJECT_ATTRIBUTES DestinationKeyName) asm("NtUnloadKey");

EXTERN_C NTSTATUS NtUnloadKey2(
IN POBJECT_ATTRIBUTES TargetKey,
IN ULONG Flags) asm("NtUnloadKey2");

EXTERN_C NTSTATUS NtUnloadKeyEx(
IN POBJECT_ATTRIBUTES TargetKey,
IN HANDLE Event OPTIONAL) asm("NtUnloadKeyEx");
...
IN PULARGE_INTEGER ByteOffset,
IN PULARGE_INTEGER Length,
IN ULONG Key) asm("NtUnlockFile");

EXTERN_C NTSTATUS NtUnlockVirtualMemory(
IN HANDLE ProcessHandle,
IN PVOID * BaseAddress,
IN PSIZE_T NumberOfBytesToUnlock,
IN ULONG LockType) asm("NtUnlockVirtualMemory");

EXTERN_C NTSTATUS NtUnmapViewOfSectionEx(
IN HANDLE ProcessHandle,
IN PVOID BaseAddress OPTIONAL,
IN ULONG Flags) asm("NtUnmapViewOfSectionEx");

EXTERN_C NTSTATUS NtUnsubscribeWnfStateChange(
IN PCWNF_STATE_NAME StateName) asm("NtUnsubscribeWnfStateChange");
...
IN ULONG CheckStamp) asm("NtUpdateWnfStateData");

EXTERN_C NTSTATUS NtVdmControl(
IN VDMSERVICECLASS Service,
IN OUT PVOID ServiceData) asm("NtVdmControl");

EXTERN_C NTSTATUS NtWaitForAlertByThreadId(
IN HANDLE Handle,
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtWaitForAlertByThreadId");

EXTERN_C NTSTATUS NtWaitForDebugEvent(
IN HANDLE DebugObjectHandle,
IN BOOLEAN Alertable,
IN PLARGE_INTEGER Timeout OPTIONAL,
OUT PVOID WaitStateChange) asm("NtWaitForDebugEvent");

EXTERN_C NTSTATUS NtWaitForKeyedEvent(
IN HANDLE KeyedEventHandle,
IN PVOID Key,
IN BOOLEAN Alertable,
IN PLARGE_INTEGER Timeout OPTIONAL) asm("NtWaitForKeyedEvent");

EXTERN_C NTSTATUS NtWaitForWorkViaWorkerFactory(
IN HANDLE WorkerFactoryHandle,
OUT PVOID MiniPacket) asm("NtWaitForWorkViaWorkerFactory");

EXTERN_C NTSTATUS NtWaitHighEventPair(
IN HANDLE EventHandle) asm("NtWaitHighEventPair");

EXTERN_C NTSTATUS NtWaitLowEventPair(
IN HANDLE EventHandle) asm("NtWaitLowEventPair");

EXTERN_C NTSTATUS NtAcquireCMFViewOwnership(
OUT BOOLEAN TimeStamp,
OUT BOOLEAN TokenTaken,
IN BOOLEAN ReplaceExisting) asm("NtAcquireCMFViewOwnership");

EXTERN_C NTSTATUS NtCancelDeviceWakeupRequest(
IN HANDLE DeviceHandle) asm("NtCancelDeviceWakeupRequest");

EXTERN_C NTSTATUS NtClearAllSavepointsTransaction(
IN HANDLE TransactionHandle) asm("NtClearAllSavepointsTransaction");

EXTERN_C NTSTATUS NtClearSavepointTransaction(
IN HANDLE TransactionHandle,
IN ULONG SavePointId) asm("NtClearSavepointTransaction");

EXTERN_C NTSTATUS NtRollbackSavepointTransaction(
IN HANDLE TransactionHandle,
IN ULONG SavePointId) asm("NtRollbackSavepointTransaction");

EXTERN_C NTSTATUS NtSavepointTransaction(
IN HANDLE TransactionHandle,
IN BOOLEAN Flag,
OUT ULONG SavePointId) asm("NtSavepointTransaction");

EXTERN_C NTSTATUS NtSavepointComplete(
IN HANDLE TransactionHandle,
IN PLARGE_INTEGER TmVirtualClock OPTIONAL) asm("NtSavepointComplete");

EXTERN_C NTSTATUS NtCreateCrossVmEvent() asm("NtCreateCrossVmEvent");

EXTERN_C NTSTATUS NtGetPlugPlayEvent(
IN HANDLE EventHandle,
IN PVOID Context OPTIONAL,
OUT PPLUGPLAY_EVENT_BLOCK EventBlock,
IN ULONG EventBufferSize) asm("NtGetPlugPlayEvent");

EXTERN_C NTSTATUS NtListTransactions() asm("NtListTransactions");

EXTERN_C NTSTATUS NtMarshallTransaction() asm("NtMarshallTransaction");

EXTERN_C NTSTATUS NtPullTransaction() asm("NtPullTransaction");

EXTERN_C NTSTATUS NtReleaseCMFViewOwnership() asm("NtReleaseCMFViewOwnership");

EXTERN_C NTSTATUS NtWaitForWnfNotifications() asm("NtWaitForWnfNotifications");

EXTERN_C NTSTATUS NtStartTm() asm("NtStartTm");

EXTERN_C NTSTATUS NtSetInformationProcess(
IN HANDLE DeviceHandle,
IN PROCESSINFOCLASS ProcessInformationClass,
IN PVOID ProcessInformation,
IN ULONG Length) asm("NtSetInformationProcess");

EXTERN_C NTSTATUS NtRequestDeviceWakeup(
IN HANDLE DeviceHandle) asm("NtRequestDeviceWakeup");

EXTERN_C NTSTATUS NtRequestWakeupLatency(
IN ULONG LatencyTime) asm("NtRequestWakeupLatency");

EXTERN_C NTSTATUS NtQuerySystemTime(
OUT PLARGE_INTEGER SystemTime) asm("NtQuerySystemTime");

EXTERN_C NTSTATUS NtManageHotPatch(
IN ULONG UnknownParameter1,
IN ULONG UnknownParameter2,
IN ULONG UnknownParameter3,
IN ULONG UnknownParameter4) asm("NtManageHotPatch");

EXTERN_C NTSTATUS NtContinueEx(
IN PCONTEXT ContextRecord,
IN PKCONTINUE_ARGUMENT ContinueArgument) asm("NtContinueEx");
...
IN PVOID StartParameter OPTIONAL,
OUT PHANDLE ThreadHandle,
OUT PCLIENT_ID ClientID) asm("RtlCreateUserThread");
